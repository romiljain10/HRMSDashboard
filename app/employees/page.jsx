...
export default function EmployeesPage() {
  ...
  return (
    <AppLayout>
      <Header title="Employee Directory" subtitle={`${location}  |  ${formatDateRangeLabel(start, end)}`} locations={locations} />
        ...
        <div style={{ background: "white", borderRadius: 8, border: "1px solid #e2e8f0", overflow: "hidden" }}>
          ...
          <div className="table-scroll">
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 640 }}>
              ...
              <tbody>
                {loading ? (
                  <SkeletonTableRows cols={11} rows={8} />
                ) : (
                paginated.map(e => (
                  <tr key={e.id} style={{ borderBottom: "1px solid #f8fafc", opacity: e.status === "Inactive" ? 0.6 : 1 }}>
                    ...
                    <td style={{ padding: "8px 12px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <Avatar id={e.id} name={e.name} size={26} fontSize={10} />
