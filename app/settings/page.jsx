        ...
        <div className="grid-2col">
          ...
          <div style={{ background:"white",borderRadius:8,border:"1px solid #e2e8f0",overflow:"hidden" }}>
            <div style={{ padding:"10px 14px",borderBottom:"1px solid #f1f5f9",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
              <div style={{ fontWeight:700,fontSize:13,color:"#0f172a",display:"flex",alignItems:"center",gap:6 }}><Users size={13}/> User Roles & Permissions</div>
              <button style={{ background:"#2563eb",color:"white",border:"none",borderRadius:5,padding:"4px 10px",fontSize:12,fontWeight:600,cursor:"pointer" }}>+ Add User</button>
            </div>
            <div className="table-scroll">
              <table style={{ width:"100%",borderCollapse:"collapse",minWidth:300 }}>
                <thead><tr style={{ background:"#f8fafc" }}>
                  {["Role","Users","Access Level"].map(h=><th key={h} style={{ padding:"7px 14px",textAlign:"left",fontSize:10,fontWeight:700,color:"#64748b",textTransform:"uppercase" }}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {roles.map((r,i)=>(
                    <tr key={i} style={{ borderBottom:"1px solid #f8fafc" }}>
                      <td style={{ padding:"8px 14px",fontSize:13,fontWeight:600,color:"#0f172a" }}>{r.role}</td>
                      <td style={{ padding:"8px 14px",fontSize:12,color:"#475569" }}>{r.users}</td>
                      <td style={{ padding:"8px 14px",fontSize:11,color:"#64748b" }}>{r.access}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
