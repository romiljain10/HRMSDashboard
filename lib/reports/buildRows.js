import { payPeriods } from "@/data/employees";

// Each entry returns { columns: [...headers], rows: [[...values], ...] }
// built from the live dataset (data from useFilteredPayrollDataset) plus,
// where noted, the static 13-period trend (no BambooHR history endpoint
// is used for that trend — see reports/payroll-hours/page.jsx).
export function buildReportRows(reportKey, data) {
  ...
  switch (reportKey) {
    case "weekly-payroll-detail":
      return {
        columns: ["Employee #", "Name", "Title", "Department", "Location", "Status", "Reg Hrs", "OT Hrs", "Holiday Hrs", "Base Rate", "Burden %", "Fully Loaded RT", "Total Cost", "Approval"],
        rows: employees.map(e => [e.employeeNumber, e.name, e.title, e.department, e.location, e.status, e.regularHours, e.otHours, e.holidayHours, e.baseRate, (e.burden * 100).toFixed(1) + "%", e.fullyLoadedRT, e.totalCost, e.approvalStatus]),
      };

    case "payroll-hours-schedule":
      return {
        columns: ["Pay Period", "Schedule", "Regular Hrs", "OT Hrs", "Holiday Hrs", "Total Hrs", "Total Cost"],
        rows: payPeriods.map(p => [p.label, p.schedule, p.regularHours, p.otHours, p.holidayHours, p.totalHours, p.totalCost]),
      };
    ...
    case "payroll-history-summary":
      return {
        columns: ["Period", "Regular Hrs", "OT Hrs", "Holiday Hrs", "Total Cost"],
        rows: payPeriods.map(p => [p.label, p.regularHours, p.otHours, p.holidayHours, p.totalCost]),
      };
