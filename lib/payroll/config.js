 ...
 */
export function computeLaborCost({ baseRate, burden, regularHours, otHours, holidayHours }) {
  const fullyLoadedRT = round2(baseRate * (1 + burden));
  const fullyLoadedOT = round2(baseRate * OT_MULTIPLIER * (1 + burden));
  const fullyLoadedHoliday = round2(baseRate * HOLIDAY_MULTIPLIER * (1 + burden));
  const totalCost = round2(
    regularHours * fullyLoadedRT + otHours * fullyLoadedOT + holidayHours * fullyLoadedHoliday
  );
