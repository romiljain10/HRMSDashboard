...
/**
 * Fetches a binary resource (e.g. an employee photo) from BambooHR.
 * Returns null on any failure (missing photo, auth failure, etc.) so
 * callers can fall back to a placeholder rather than erroring the page.
 */
export async function bambooGetBinary(path) {
  const { subdomain, apiKey, baseUrl } = getConfig();
  const authHeader = "Basic " + Buffer.from(`${apiKey}:x`).toString("base64");
